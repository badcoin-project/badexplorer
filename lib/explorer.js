...
const rpc_queue = async.queue((task_params, cb) => {
  ...
  client.cmd([task_params], function(err, response) {
    if (err)
      return done(`${settings.localization.ex_error}: ${settings.localization.check_console}`, null);
...
function rpcCommand(params, cb) {
  if (!Array.isArray(params)) {
    if (params && params.method)
      ...
      console.log('Invalid rpcCommand params:', params);
      return cb(`${settings.localization.ex_error}: ${settings.localization.check_console}`);
    }
  }

  if (!params[0] || !params[0].method) {
    console.log('Invalid rpcCommand command:', params);
    return cb(`${settings.localization.ex_error}: ${settings.localization.check_console}`);
  }

  rpc_queue.push({method: params[0].method, params: params[0].parameters || []}, function(err, result) {
    if (err)
      return cb(err);
    else
      return cb(result);
...
get_blockchaininfo: function(cb) {
  var cmd = prepareRpcCommand(settings.api_cmds.getblockchaininfo);

  if (settings.api_cmds.use_rpc === true) {
    rpcCommand([{method: cmd.method, parameters: cmd.parameters}], function(response) {
      if (!response || response == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
        return cb(null);
      else
        return cb(response);
    });
    ...
    if (cmd.method == 'getblockchaininfo') {
      ...
      request({uri: uri, json: true}, function (error, response, body) {
        if (error || !body || body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
          return cb(null);
        else
          return cb(body);
      });
