...
module.exports = {
  ...
  const uri =
    'mongodb://' +
    encodeURIComponent(settings.dbsettings.user) +
    ':' +
  ...
  get_last_txs: function(start, length, min, internal, cb) {
    this.get_last_txs_ajax(start, length, min, function(txs, count) {
      ...
      async.timesSeries(txs.length, function(i, loop) {
        // Capture the tx before entering asynchronous extracted-by processing.
        // A missing/sparse result must not terminate an explorer worker.
        const tx = txs[i];
        ...
        get_extracted_by_addresses(show_extracted_by, internal, tx, function(extracted_by_addresses) {
          if (internal) {
            ...
            row.push((tx.total / 100000000));
            row.push(tx.timestamp);

            if (settings.block_page.multi_algorithm.show_algo == true)
              row.push('algo:' + (tx.algo == null ? '' : tx.algo));
            ...
            if (settings.block_page.multi_algorithm.show_algo == true)
              data_entry.algo = (tx.algo == null ? '' : tx.algo);
